...
@section('title', __('user.admin_profile'))
...
<div class="card shadow mb-4">
   ...
   <div class="card-header py-3">
     ...
     <ul class="breadcrumbs">
         <li><a href="{{route('admin')}}" style="color:#999">{{ __('user.dashboard') }}</a></li>
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <div class="image">
                        @if($profile->photo)
                        <img class="card-img-top img-fluid rounded-circle mt-4" style="border-radius:50%;height:80px;width:80px;margin:auto;" src="{{$profile->photo}}" alt="{{ __('user.profile_picture') }}">
                        @else 
                        <img class="card-img-top img-fluid rounded-circle mt-4" style="border-radius:50%;height:80px;width:80px;margin:auto;" src="{{asset('backend/img/avatar.png')}}" alt="{{ __('user.profile_picture') }}">
                        @endif
                    </div>
            ...
            <div class="col-md-8">
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('user-profile-update',$profile->id)}}">
                    ...
                    <div class="form-group">
                      ...
                      <input id="inputTitle" type="text" name="name" placeholder="{{ __('user.enter_name') }}"  value="{{$profile->name}}" class="form-control">
                      @error('name')
                      ...
                      <div class="form-group">
                          <label for="inputEmail" class="col-form-label">{{ __('user.email') }}</label>
                        <input id="inputEmail" disabled type="email" name="email" placeholder="{{ __('user.enter_email') }}"  value="{{$profile->email}}" class="form-control">
                        @error('email')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        <button type="submit" class="btn btn-success btn-sm">{{ __('user.update') }}</button>
                </form>
            </div>
        </div>
   </div>
